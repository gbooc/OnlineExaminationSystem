...
function SubmitExam(isGoodBye, isSubmitted, isQuickExam) {
    ...
    //Override mcHasEmpty / No multiple choice found in the question
    if (hasMC == 0)
       mcHasEmpty = 1;
...
function resultToExcel() {
    
    window.location.href = "/Results/ExportToExcel?ExamID=" + $("#txt_ExamID").val();
}

function autoCorrect(question, answer, examtype) {

    if ($("#txt-hasexam").val() !== "") {

        $.ajax({
            url: '/Examinations/GetRightAnswer',
            type: 'POST',
            async: false,
            data: JSON.stringify({ QID: question, Answer: answer.value, ExamType: examtype }),
            dataType: "json",
            traditional: true,
            contentType: 'application/json;',
            cache: false,
            success: function (IsCorrect) {
                if (!IsCorrect)
                    $("#incorrect_" + question).show();
                else
                    $("#incorrect_" + question).hide();
